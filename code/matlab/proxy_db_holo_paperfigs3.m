...
%shortlist = find(err <= 1);  So_all = So;
%So = So_all(shortlist);
export = 0
% define key arrays
ref = {So.reference};
...
%  SYNTHESIS FIGURES
% =================================================
load ../data/NCEP_OIv2_SODA_SSS_1981_2010_djf.mat
ilon = (lon>=100& lon <=300); ilat = (abs(lat)<=40);
lon_tp = lon(ilon); lat_tp = lat(ilat);
%h = fspecial('disk', 5);
%sss_enso_smooth = filter2(h, sss_enso);
...
m_coast('patch',ligr);
m_grid('box','off','xtick',6,'ytick',7,'xlabeldir','middle', 'fontsize',8,'fontname','Palatino');
hc = colorbar2('horiz','\delta^{18}O (permil)');
% add boxes.
h1 = m_line([120 120 180 180 120], [-20 0 0 -20 -20],'color',bck,'linewidth',2,'linestyle','-.');
h3 = m_line([270 270 280 280 270], [-10 0 0 -10 -10],'color',bck,'linewidth',2,'linestyle','-.');
h2 = m_line([190 190 240 240 190], [-5  5 5  -5  -5],'color',bck,'linewidth',2,'linestyle','-.');
% add proxies
for r=1:no
...
set(object_h(6),'MarkerFaceColor','w','linewidth',2),
set(object_h(4),'MarkerFaceColor','w','linewidth',2);
set(legend_h,style_l{:},'Outerposition',[0.2 0.1 0.6 0.3]),
ht = title(['Proxy locations & El Niño composite \delta^{18}O']);
set(ht,'FontName','Palatino','Fontsize',16,'FontWeight','bold');
if export
    hepta_figprint('../figs/holocene_proxy_locs_boxes_d18Ofield');
end
% ===================================
...
p.fontsize = 10; p.fontname = 'Palatino';
...
for jr = 1:3
    ...
    for r = reg_ind{jr}'
        ...
        line([So(r).year_i/1000 So(r).year_f/1000],[q(3) q(3)],'color',col{r},'linewidth',2)
    ...
    xlim(ylims); xlabel(region{jr},'fontsize',14,'fontweight','bold','fontname','Palatino'),
    ...
    for m = 1:nm
        ...
        set(hmh_m(m),'color',cmap(m,:),'linestyle','-','marker','none','linewidth',2);
    ...
    for m = 1:nm
        ...
        hpi_m(m) = plot(ratio_vec(rind),f(rind));
        set(hpi_m(m),'color',cmap(m,:),'linestyle','-','marker','none','linewidth',2);
...
% p.fontsize = 8; p.fontname = 'Palatino';
% %p.select('all');
% %p.identify()
%
% for jr = 1:3
