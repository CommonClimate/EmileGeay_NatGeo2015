...
for m = 1:2*nm
    ...
    for j = 1:3
        hm(m) = line(Xm(m,j),Ym(m,j),'MarkerFaceColor',colm(m,:),'marker',mrkr{m},'Markersize',10,'MarkerEdgeColor',bck);
...
for m = 1:2*nm
    ...
    for j = 1:3
        hm(m) = line(Xms(m,j),Yms(m,j),'MarkerFaceColor',colm(m,:),'marker',mrkr{m},'Markersize',10,'MarkerEdgeColor',bck);
...
% ===================================
PMIP3 = load('../models/coral_regional_avg.mat');
...
for ie = 1:nexp
    ...
    for im = 1:nm
        ...
        fname = ['../figs/wavelet_' exp{ie} '_' model_names{ie}{im} '_center.pdf'];
        % plot
        fig('ENSO vs annual cycle'),clf
        subplot(2,1,1)
        plot(t,X);
